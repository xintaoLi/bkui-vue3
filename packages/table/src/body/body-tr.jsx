...
export default {
  ...
  render() {
    ...
    const getTdContent = () => {
      return colgroups.map(column => {
        const tdProps = {
          ...
          ...convertToV3Event({
            [EMIT_EVENTS.EXPAND_ROW_CHANGE]: () => expandRowChange(rowData, rowIndex),
          })
        };
    ...
    if (this.isVirtualScroll) {
      ...
      result = <VueDomResizeObserver {...props}>{getTdContent()}</VueDomResizeObserver>;
    } else {
      const props = {
        class: this.trClass,
        [COMPS_CUSTOM_ATTRS.BODY_ROW_KEY]: this.currentRowKey,
