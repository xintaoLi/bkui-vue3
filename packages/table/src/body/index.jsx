...
import { getDomResizeObserverCompKey, getFixedTotalWidthByColumnKey, clsName } from '../util';
import { getValByUnit } from '../utils/index.js';
import emitter from '../mixins/emitter';
import { COMPS_NAME, EMIT_EVENTS, COLUMN_TYPES, EXPAND_TRIGGER_TYPES } from '../util/constant';
...
export default {
  ...
  computed: {
    ...
    columnCollection() {
      ...
      colgroups.forEach(col => {
        ...
        if (fixed) {
          ...
          if ((fixed === 'left' && columnIndex > 0) || (fixed === 'right' && columnIndex < colgroups.length - 1)) {
    ...
    // expanded row keys
    expandedRowkeys() {
      return this.isControlledExpand ? this.expandOption.expandedRowKeys : this.internalExpandRowkeys;
    },
    ...
    isCheckboxSelectedAll() {
      if (this.allRowKeys.length > 0) {
        if (
          this.internalCheckboxSelectedRowKeys.length + this.disableCheckboxUnselectedRowKeys.length ===
    ...
    isCheckboxIndeterminate() {
      ...
      return internalCheckboxSelectedRowKeys.length > 0 && internalCheckboxSelectedRowKeys.length < allRowKeys.length;
    },
  ...
  methods: {
    ...
    expandRowChange(rowData, rowIndex) {
      const { expandOption, internalExpandRowkeys, expandedRowkeys, rowKeyFieldName } = this;
    ...
    initInternalRadioSelectedRowKey() {
      ...
      this.internalRadioSelectedRowKey = isControlledRadio ? selectedRowKey : defaultSelectedRowKey;
    },
      ...
      <tbody>
        ...
        <tr style='height:0;'>
          {colgroups.map(column => {
            const measureTdProps = {
              ...
              tagName: 'td',
              id: column.key,
              'onDom-resize-change': tdSizeChange,
              style: {
                padding: 0,
                border: 0,
                height: 0,
        ...
        {actualRenderTableData.map((rowData, rowIndex) => {
          ...
          if (showVirtualScrollingPlaceholder) {
            ...
            if (virtualScrollRepeatRenderedRowKeys.indexOf(rowData[this.rowKeyFieldName]) != -1) {
              return [
                // body tr
                <BodyTr {...trProps} />,
              ];
