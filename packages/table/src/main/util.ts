...
import { isPx, isScale } from '../ui/dom';
...
function queryCellElement(cell: HTMLTableCellElement, selector: string) {
  return cell.querySelector('.bk-cell' + selector) as HTMLElement | null;
}
...
export function getColReMinWidth(params: {
  ...
  // 默认最小宽处理
  if (hasEllipsis) {
    const checkboxIconWidth = getPaddingLeftRightSize(queryCellElement(cell, '--title>.bk-cell--checkbox'));
    const requiredIconWidth = getElemenMarginWidth(queryCellElement(cell, '>.bk-cell--required-icon'));
    const editIconWidth = getElemenMarginWidth(queryCellElement(cell, '>.bk-cell--edit-icon'));
    const prefixIconWidth = getElemenMarginWidth(queryCellElement(cell, '>.bk-cell-title-prefix-icon'));
    const suffixIconWidth = getElemenMarginWidth(queryCellElement(cell, '>.bk-cell-title-suffix-icon'));
    const sortIconWidth = getElemenMarginWidth(queryCellElement(cell, '>.bk-cell--sort'));
    const filterIconWidth = getElemenMarginWidth(queryCellElement(cell, '>.bk-cell--filter'));
    mWidth +=
      checkboxIconWidth +
      requiredIconWidth +
      editIconWidth +
