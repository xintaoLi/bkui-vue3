  ...
  setup(props, context) {
    ...
    const computeValidOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.validConfig, true),
        props.validConfig,
      ) as VxeTablePropTypes.ValidOpts;
    });

    const computeSXOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.scrollX, true),
        props.scrollX,
      ) as VxeTablePropTypes.SXOpts;
    });

    const computeSYOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.scrollY, true),
        props.scrollY,
      ) as VxeTablePropTypes.SYOpts;
    });
    ...
    const computeColumnOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.columnConfig, true),
        props.columnConfig,
      ) as VxeTablePropTypes.ColumnOpts;
    });

    const computeRowOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.rowConfig, true),
        props.rowConfig,
      ) as VxeTablePropTypes.RowOpts;
    });

    const computeResizeOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.resizeConfig, true),
        props.resizeConfig,
      ) as VxeTablePropTypes.ResizeOpts;
    });

    const computeResizableOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.resizableConfig, true),
        props.resizableConfig,
      ) as VxeTablePropTypes.ResizableOpts;
    });

    const computeSeqOpts = computed(() => {
      return Object.assign(
        { startIndex: 0 },
        XEUtils.clone(getConfig().table.seqConfig, true),
        props.seqConfig,
      ) as VxeTablePropTypes.SeqOpts;
    });

    const computeRadioOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.radioConfig, true),
        props.radioConfig,
      ) as VxeTablePropTypes.RadioOpts;
    });

    const computeCheckboxOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.checkboxConfig, true),
        props.checkboxConfig,
      ) as VxeTablePropTypes.CheckboxOpts;
    });
    ...
    computeTooltipOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().tooltip, true),
        XEUtils.clone(getConfig().table.tooltipConfig, true),
        props.tooltipConfig,
      );
    });
    ...
    const computeEditOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.editConfig, true),
        props.editConfig,
      ) as VxeTablePropTypes.EditOpts;
    });

    const computeSortOpts = computed(() => {
      return Object.assign(
        { orders: ['asc', 'desc', null] },
        XEUtils.clone(getConfig().table.sortConfig, true),
        props.sortConfig,
      ) as VxeTablePropTypes.SortOpts;
    });

    const computeFilterOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.filterConfig, true),
        props.filterConfig,
      ) as VxeTablePropTypes.FilterOpts;
    });

    const computeMouseOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.mouseConfig, true),
        props.mouseConfig,
      ) as VxeTablePropTypes.MouseOpts;
    });

    const computeAreaOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.areaConfig, true),
        props.areaConfig,
      ) as VxeTablePropTypes.AreaOpts;
    });

    const computeKeyboardOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.keyboardConfig, true),
        props.keyboardConfig,
      ) as VxeTablePropTypes.KeyboardOpts;
    });
    ...
    const computeFNROpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.fnrConfig, true),
        props.fnrConfig,
      ) as VxeTablePropTypes.FNROpts;
    });

    const computeMenuOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.menuConfig, true),
        props.menuConfig,
      ) as VxeTablePropTypes.MenuOpts;
    });
    ...
    const computeExportOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.exportConfig, true),
        props.exportConfig,
      ) as VxeTablePropTypes.ExportOpts;
    });

    const computeImportOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.importConfig, true),
        props.importConfig,
      ) as VxeTablePropTypes.ImportOpts;
    });

    const computePrintOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.printConfig, true),
        props.printConfig,
      ) as VxeTablePropTypes.PrintOpts;
    });

    const computeExpandOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.expandConfig, true),
        props.expandConfig,
      ) as VxeTablePropTypes.ExpandOpts;
    });

    const computeTreeOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.treeConfig, true),
        props.treeConfig,
      ) as VxeTablePropTypes.TreeOpts;
    });

    const computeEmptyOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.emptyRender, true),
        props.emptyRender,
      ) as VxeTablePropTypes.EmptyOpts;
    });

    const computeLoadingOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().table.loadingConfig, true),
        props.loadingConfig,
      ) as VxeTablePropTypes.LoadingOpts;
    });
    ...
    const computeAutoWidthColumnList = computed(() => {
      ...
      return tableColumn.length || visibleColumn.length
        ? visibleColumn.filter(column => column.width === 'auto' || column.minWidth === 'auto')
        : [];
    });
    ...
    const setMerges = (
      merges: VxeTableDefines.MergeOptions | VxeTableDefines.MergeOptions[],
      mList: VxeTableDefines.MergeItem[],
      rowList?: any[],
    ) => {
      if (merges) {
        const { treeConfig } = props;
        ...
        if (el) {
          autoWidthColumnList.forEach(column => {
            ...
            if (firstCellEl) {
              ...
              paddingSize =
                Math.floor(XEUtils.toNumber(cellStyle.paddingLeft) + XEUtils.toNumber(cellStyle.paddingRight)) + 2;
            }
    ...
    const autoCellWidth = () => {
      ...
      meanWidth =
        remainWidth > 0
          ? Math.floor(remainWidth / (scaleMinList.length + pxMinList.length + autoMinList.length + remainList.length))
          : 0;
      if (fit) {
        if (remainWidth > 0) {
          scaleMinList
          ...
          const handleFilter = (row: any) => {
            return filterColumns.every(({ column, valueList, itemList }) => {
              ...
              const tdFilterMethod = compConf
                ? compConf.tableFilterDefaultMethod || compConf.defaultTableFilterMethod || compConf.defaultFilterMethod
                : null;
              const cellValue = getCellValue(row, column);
              if (filterMethod) {
                return itemList.some(item =>
                  filterMethod({ value: item.value, option: item, cellValue, row, column, $table: $xeTable }),
                );
              } else if (compFilterMethod) {
                return itemList.some(item =>
                  compFilterMethod({ value: item.value, option: item, cellValue, row, column, $table: $xeTable }),
                );
              } else if (allFilterMethod) {
                return allFilterMethod({ options: itemList, values: valueList, cellValue, row, column });
              } else if (tdFilterMethod) {
                return itemList.some(item =>
                  tdFilterMethod({ value: item.value, option: item, cellValue, row, column, $table: $xeTable }),
                );
              }
          ...
          if (layout === 'header') {
            ...
            if (isGroup && listElem) {
              XEUtils.arrayEach(listElem.querySelectorAll('.col--group'), (thElem: any) => {
                ...
                if (colNode) {
                  ...
                  const cellOverflow = XEUtils.isBoolean(showHeaderOverflow)
                    ? showHeaderOverflow
            ...
            if (tableElem) {
              ...
              tableElem.style.paddingRight =
                scrollbarWidth && fixedType && (browse['-moz'] || browse.safari) ? `${scrollbarWidth}px` : '';
            }
            if (emptyBlockElem) {
    ...
    const handleReserveStatus = () => {
      const { treeConfig } = props;
      const { expandColumn, currentRow, selectCheckboxMaps, selectRadioRow, rowExpandedMaps, treeExpandedMaps } =
        reactData;
    ...
    const handleDefaultTreeExpand = () => {
      ...
      if (treeConfig) {
        ...
        if (expandAll) {
          ...
          const rowkey = getRowkey($xeTable);
          expandRowKeys.forEach((rowid: any) => {
            const matchObj = XEUtils.findTree(tableFullData, item => rowid === XEUtils.get(item, rowkey), {
              children: childrenField,
            });
            if (matchObj) {
    ...
    const parseColumns = () => {
      ...
      if (isGroup) {
        ...
        XEUtils.eachTree(collectColumn, (column, index, items, path, parent) => {
          ...
          if (isColGroup) {
            column.visible = !!XEUtils.findTree(column.children, subColumn =>
    ...
    const handleBaseTreeExpand = (rows: any[], expanded: boolean) => {
      ...
      let validRows = toggleMethod
        ? rows.filter((row: any) =>
            toggleMethod({ $table: $xeTable, expanded, column: treeNodeColumn, columnIndex, $columnIndex, row }),
          )
        : rows;
      if (accordion) {
    ...
    tableMethods = {
      ...
      loadTreeChildren(row, childRecords) {
        ...
        return tableMethods.createData(childRecords).then(rows => {
          if (keepSource) {
            const rowid = getRowid($xeTable, row);
            const matchObj = XEUtils.findTree(tableSourceData, item => rowid === getRowid($xeTable, item), {
              children: childrenField,
            });
            if (matchObj) {
      ...
      getRadioReserveRecord(isFull) {
        ...
        if (radioOpts.reserve && radioReserveRow) {
          ...
          if (isFull) {
            ...
            const rowkey = getRowkey($xeTable);
            if (treeConfig) {
              const matchObj = XEUtils.findTree(afterFullData, row => rowid === XEUtils.get(row, rowkey), {
                children: childrenField,
              });
              if (matchObj) {
                return radioReserveRow;
      ...
      setCurrentRow(row) {
        ...
        if (rowOpts.isCurrent || props.highlightCurrentRow) {
          if (el) {
            XEUtils.arrayEach(el.querySelectorAll(`[rowid="${getRowid($xeTable, row)}"]`), elem =>
      ...
      updateFooter() {
        ...
        if (showFooter && footerData && footerData.length) {
          ...
          footData = visibleColumn.length
            ? footerMethod({ columns: visibleColumn, data: afterFullData, $table: $xeTable, $grid: $xeGrid })
            : [];
        }
      ...
      if (internalData.isActivated) {
        tablePrivateMethods.preventEvent(evnt, 'event.keydown', null, () => {
          ...
          if (operCtxMenu) {
            ...
            (selected.row || actived.row || (treeConfig && (rowOpts.isCurrent || highlightCurrentRow) && currentRow))
          ) {
            // 退出选中
            if (hasCtrlKey) {
              // 如果是激活编辑状态，则取消编辑
            ...
            // 如果树形表格回退键关闭当前行返回父节点
            const { parent: parentRow } = XEUtils.findTree(
              internalData.afterTreeFullData,
    ...
    const handleTooltip = (
      ...
      overflowElem: HTMLElement,
      tipElem: HTMLElement | null,
      params: any,
    ) => {
      params.cell = cell;
      const { tooltipStore } = reactData;
      ...
      const content = useCustom
        ? customContent
        : XEUtils.toString(column.type === 'html' ? overflowElem.innerText : overflowElem.textContent).trim();
    ...
    tablePrivateMethods = {
        ...
        if (el) {
          ...
          return Math.floor(
            $xeGrid
              ...
              : XEUtils.toNumber(getComputedStyle(parentElem).height) - parentPaddingSize,
          );
        }
        return 0;
      },
      ...
      defineField(records) {
        ...
        return records.map(record => {
          ...
          const otherFields: (string | undefined)[] = [
      ...
      preventEvent(evnt, type, args, next, end) {
        ...
        if (
          !evntList.some(
            func => func(Object.assign({ $grid: $xeGrid, $table: $xeTable, $event: evnt }, args)) === false,
          )
        ) {
          if (next) {
            rest = next();
        ...
        if (!checkStrictly) {
          ...
          if (checkField) {
            ...
            if (treeConfig) {
              if (indeterminateField) {
                ...
                isIndeterminate =
                  !isAllSelected &&
                  afterFullData.some(
                    row => XEUtils.get(row, checkField) || !!treeIndeterminateMaps[getRowid($xeTable, row)],
                  );
              }
            } else {
              if (indeterminateField) {
                isIndeterminate =
                  !isAllSelected &&
                  afterFullData.some(row => XEUtils.get(row, checkField) || XEUtils.get(row, indeterminateField));
              } else {
            ...
            if (treeConfig) {
              ...
              isIndeterminate =
                !isAllSelected && afterFullData.some(row => selectCheckboxMaps[getRowid($xeTable, row)]);
            }
          }
      ...
      handleSelectRow({ row }, value, isForce) {
        ...
        if (checkField) {
          if (treeConfig && !checkStrictly) {
            ...
            // 如果存在父节点，更新父节点状态
            const matchObj = XEUtils.findTree(afterFullData, item => $xeTable.eqRow(item, row), {
              children: childrenField,
            });
            if (matchObj && matchObj.parent) {
              ...
              const indeterminatesItem = XEUtils.find(
                matchObj.items,
      ...
      scrollToTreeRow(row) {
        ...
        if (treeConfig) {
          const treeOpts = computeTreeOpts.value;
          const childrenField = treeOpts.children || treeOpts.childrenField;
          const matchObj = XEUtils.findTree(tableFullData, item => $xeTable.eqRow(item, row), {
            children: childrenField,
          });
          if (matchObj) {
    ...
    const renderEmptyContenet = () => {
      ...
      if (slots.empty) {
        ...
        const rtEmptyView = compConf
          ? compConf.renderTableEmpty || compConf.renderTableEmptyView || compConf.renderEmpty
          : null;
        if (rtEmptyView) {
        ...
        [
          ...
          VxeUITooltipComponent
            ? h('div', {}, [
                ...
                (validOpts.message === 'default' ? !height : validOpts.message === 'tooltip')
                  ? h(VxeUITooltipComponent, {
                      ref: refValidTooltip,
                      class: [
                        {
    ...
    onMounted(() => {
      nextTick(() => {
        ...
        if (process.env.VUE_APP_VXE_ENV === 'development') {
          ...
          if (
            props.editConfig &&
            (editOpts.showStatus || editOpts.showUpdateStatus || editOpts.showInsertStatus) &&
            !props.keepSource
          ) {
            warnLog('vxe.error.reqProp', ['keep-source']);
          }
          ...
          if (
            ...
            !XEUtils.includeArrays(XEUtils.keys(importOpts._typeMaps), importOpts.types)
          ) {
            warnLog('vxe.error.errProp', [
              `export-config.types=${importOpts.types.join(',')}`,
              importOpts.types
                .filter((type: string) => XEUtils.includes(XEUtils.keys(importOpts._typeMaps), type))
                .join(',') || XEUtils.keys(importOpts._typeMaps).join(','),
            ]);
          }
          if (
            exportConfig &&
            ...
            !XEUtils.includeArrays(XEUtils.keys(exportOpts._typeMaps), exportOpts.types)
          ) {
            warnLog('vxe.error.errProp', [
              `export-config.types=${exportOpts.types.join(',')}`,
              exportOpts.types
                .filter((type: string) => XEUtils.includes(XEUtils.keys(exportOpts._typeMaps), type))
                .join(',') || XEUtils.keys(exportOpts._typeMaps).join(','),
            ]);
          }
        }
