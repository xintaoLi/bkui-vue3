...
import { updateCellTitle, getPropClass } from '../ui/dom';
import { isEnableConf } from '../ui/utils';
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const { refTableHeader, refTableBody, refTableFooter, refTableLeftBody, refTableRightBody, refValidTooltip } =
      $xeTable.getRefMaps();
    const {
      computeEditOpts,
      computeMouseOpts,
    ...
    const renderColumn = (
      ...
      const { tableData, overflowX, scrollYLoad, currentColumn, mergeList, editStore, isAllOverflow, validErrorMaps } =
        tableReactData;
      const { afterFullData } = tableInternalData;
      ...
      const cellOverflow =
        XEUtils.isUndefined(showOverflow) || XEUtils.isNull(showOverflow) ? allColumnOverflow : showOverflow;
      ...
      const showValidTip =
        ...
        (validOpts.message === 'default' ? height || tableData.length > 1 : validOpts.message === 'inline');
        ...
        'td',
        {
          class: [
            ...
            colid,
            {
              ...
              'col--dirty': isDirty,
              'col--active':
                editConfig && isEdit && actived.row === row && (actived.column === column || editOpts.mode === 'row'),
