...
import { getFuncText, isEnableConf, formatText, eqEmptyValue } from '../ui/utils';
import { updateCellTitle } from '../ui/dom';
...
function renderTitlePrefixIcon(params: VxeTableDefines.CellRenderHeaderParams) {
  ...
  return titlePrefix
    ? [
        h('i', {
          class: ['bk-cell-title-prefix-icon', titlePrefix.icon || getIcon().TABLE_TITLE_PREFIX],
...
function renderTitleSuffixIcon(params: VxeTableDefines.CellRenderHeaderParams) {
  ...
  return titleSuffix
    ? [
        h('i', {
          class: ['bk-cell-title-suffix-icon', titleSuffix.icon || getIcon().TABLE_TITLE_SUFFIX],
          onMouseenter(evnt: MouseEvent) {
...
function renderTitleContent(
  params: VxeTableDefines.CellRenderHeaderParams,
  content: VxeComponentSlotType | VxeComponentSlotType[],
) {
  const { $table, column } = params;
...
export const Cell = {
  createColumn(
    ...
    columnOpts: VxeTableDefines.ColumnOptions | VxeTableDefines.ColumnInfo,
  ) {
  ...
  renderSeqHeader(params: VxeTableDefines.CellRenderHeaderParams) {
    ...
    return renderTitleContent(
      params,
      headerSlot ? $table.callSlot(headerSlot, params) : formatText(column.getTitle(), 1),
    );
  },
  renderSeqCell(params: VxeTableDefines.CellRenderBodyParams) {
    ...
    return [
      formatText(seqMethod ? seqMethod(params) : treeConfig ? seq : (seqOpts.startIndex || 0) + (seq as number), 1),
    ];
  },
  ...
  renderRadioCell(params: VxeTableDefines.CellRenderBodyParams) {
    ...
    if (isVisible) {
      radioVNs.push(
        h('span', {
          class: ['bk-radio--icon', isChecked ? getIcon().TABLE_RADIO_CHECKED : getIcon().TABLE_RADIO_UNCHECKED],
        }),
      );
    }
    ...
    if (showIcon) {
      return [
        h(
          'span',
          {
            class: ['bk-cell--sort', `bk-cell--sort-${iconLayout}-layout`],
          },
          [
            h('i', {
              class: [
          ...
          h(
            ...
            [
              h('i', {
                class: [
                  'bk-filter--btn',
                  hasFilter ? iconMatch || getIcon().TABLE_FILTER_MATCH : iconNone || getIcon().TABLE_FILTER_NONE,
                ],
      ...
      isEnableConf(editConfig)
        ? [
            isRequired && editOpts.showAsterisk
              ? h('i', {
                  class: 'bk-cell--required-icon',
                })
              : null,
            isEnableConf(editRender) && editOpts.showIcon
              ? h('i', {
                  class: ['bk-cell--edit-icon', editOpts.icon || getIcon().TABLE_EDIT],
                })
              : null,
          ]
        : []
  ...
  renderCellEdit(params: VxeTableDefines.CellRenderBodyParams) {
    ...
    return Cell.runRenderer(
      params,
      isEnableConf(editRender) && actived && actived.row === params.row && actived.column === params.column,
    );
  },
