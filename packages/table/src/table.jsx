...
import { requestAnimationTimeout, cancelAnimationTimeout } from './utils/request-animation-timeout';
import { isInputKeyCode } from './utils/event-key-codes';
import Hooks from './utils/hooks-manager';
...
import VueDomResizeObserver from './comps/resize-observer';
import Contextmenu from './contextmenu';
import ColumnResizer from './column-resizer';
import { defineComponent, getCurrentInstance, toRefs, watch, withDirectives } from 'vue';
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      /*
            列配置变化次数
            依赖columns 配置渲染，都需要重新计算：粘性布局时，重新触发 dom-resize-change 事件
            */
  ...
  computed: {
    ...
    virtualScrollBufferCount() {
      ...
      const { virtualScrollOption, defaultVirtualScrollBufferScale, virtualScrollVisibleCount } = this;

      if (virtualScrollOption) {
        const { bufferScale } = virtualScrollOption;

        let realBufferScale = isNumber(bufferScale) && bufferScale > 0 ? bufferScale : defaultVirtualScrollBufferScale;
    ...
    virtualScrollVisibleCount() {
      ...
      const { isVirtualScroll, virtualScrollOption, defaultVirtualScrollMinRowHeight, maxHeight, tableOffestHeight } =
        this;
    ...
    // has fixed column
    hasFixedColumn() {
      return this.colgroups.some(x => x.fixed === COLUMN_FIXED_TYPE.LEFT || x.fixed === COLUMN_FIXED_TYPE.RIGHT);
    },
    // has left fixed column
    hasLeftFixedColumn() {
      return this.colgroups.some(x => x.fixed === COLUMN_FIXED_TYPE.LEFT);
    },
    // has right fixed column
    hasRightFixedColumn() {
      return this.colgroups.some(x => x.fixed === COLUMN_FIXED_TYPE.RIGHT);
    },
    // is editing cell
    isCellEditing() {
      const { editingCell } = this;

      return !isEmptyValue(editingCell.rowKey) && !isEmptyValue(editingCell.colKey);
    },
    // has edit column
    hasEditColumn() {
    ...
    enableCellSelection() {
      ...
      if (isEmptyValue(rowKeyFieldName)) {
        result = false;
      } else if (cellSelectionOption && isBoolean(cellSelectionOption.enable) && cellSelectionOption.enable === false) {
        result = false;
      }
      return result;
    },
  ...
  methods: {
    ...
    selectedAllChange({ isSelected }) {
      this.broadcast(COMPS_NAME.VE_TABLE_BODY, EMIT_EVENTS.CHECKBOX_SELECTED_ALL_CHANGE, {
        isSelected,
      });
    },
    ...
    setSelectedAllInfo({ isSelected, isIndeterminate }) {
      this.broadcast(COMPS_NAME.VE_TABLE_HEADER_CHECKBOX_CONTENT, EMIT_EVENTS.CHECKBOX_SELECTED_ALL_INFO, {
        isSelected,
        isIndeterminate,
      });
    },
    ...
    headerIndicatorColKeysChange({ startColKey, endColKey }) {
      ...
      this.headerIndicatorColKeys.startColKeyIndex = colgroups.findIndex(x => x.key === startColKey);
      this.headerIndicatorColKeys.endColKey = endColKey;
      this.headerIndicatorColKeys.endColKeyIndex = colgroups.findIndex(x => x.key === endColKey);
    },
    ...
    setCellSelectionByAutofill() {
      ...
      else if (currentCellSelectionType === CURRENT_CELL_SELECTION_TYPES.SINGLE) {
    ...
    dealKeydownEvent(event) {
      const { colgroups, cellSelectionData, enableStopEditing, isCellEditing } = this;
    ...
    rowToVisible(keyCode, nextRowKey) {
      ...
      const { isVirtualScroll, headerTotalHeight, footerTotalHeight } = this;

      const { clientHeight: containerClientHeight, scrollTop: containerScrollTop } = tableContainerRef;

      const nextRowEl = this.$el.querySelector(`tbody tr[${COMPS_CUSTOM_ATTRS.BODY_ROW_KEY}="${nextRowKey}"]`);
      ...
      if (nextRowEl) {
        ...
        // arrow down
          ...
          if (isVirtualScroll) {
            ...
            diff = trOffsetTop + trClientHeight + footerTotalHeight - (containerClientHeight + containerScrollTop);
          }
    ...
    getVirtualScrollAboveCount() {
      ...
      if (isVirtualScroll) {
        result = Math.min(virtualScrollStartIndex, virtualScrollBufferCount);
      }
      return result;
    },
    ...
    getVirtualScrollBelowCount() {
      ...
      if (isVirtualScroll) {
        result = Math.min(tableData.length - virtualScrollEndIndex, virtualScrollBufferCount);
      }
    ...
    getVirtualViewPhantom() {
      ...
      if (isVirtualScroll || (hasLeftFixedColumn && expandOption)) {
        const props = {
          ...
          'onDom-resize-change': ({ width }) => {
            console.log('tableViewportWidth')
        ...
        content = (
          <div
            ...
            class={[clsName('virtual-phantom'), isVirtualScroll ? clsName('virtual-scroll') : '']}
          >
    ...
    initVirtualScrollPositions() {
      if (this.isVirtualScroll) {
        const { virtualScrollOption, rowKeyFieldName, tableData, defaultVirtualScrollMinRowHeight } = this;
    ...
    bodyRowHeightChange({ rowKey, height }) {
      ...
      let oldHeight = this.virtualScrollPositions[index]?.height ?? 0;
    ...
    setVirtualPhantomHeight() {
      ...
      if (this.virtualScrollPositions.length) {
        totalHeight = this.virtualScrollPositions[this.virtualScrollPositions.length - 1].bottom;
      }

      this.$refs[this.virtualPhantomRef].style.height = totalHeight + 'px';
    },
    ...
    setVirtualScrollStartOffset() {
      ...
      if (start >= 1) {
        let size =
          ...
          (this.virtualScrollPositions[start - aboveCount] ? this.virtualScrollPositions[start - aboveCount].top : 0);
    ...
    getVirtualScrollStartIndex(scrollTop = 0) {
      return this.virtualScrollBinarySearch(this.virtualScrollPositions, scrollTop);
    },
    ...
    tableContainerVirtualScrollHandler(tableContainerRef) {
      const { virtualScrollVisibleCount: visibleCount, virtualScrollOption } = this;
      ...
      if (!this.showVirtualScrollingPlaceholder) {
        ...
        if (bodyElement) {
          bodyElement.renderingRowKeys(
            this.allRowKeys.slice(visibleStartIndex - visibleAboveCount, visibleEndIndex + visibleBelowCount),
          );
        }
      }
    ...
    initVirtualScroll() {
      if (this.isVirtualScroll) {
        ...
        this.virtualScrollEndIndex = startIndex + this.virtualScrollVisibleCount;
    ...
    setScrolling(tableContainerRef) {
      if (this.hasFixedColumn) {
        ...
        const { previewTableContainerScrollLeft: previewScrollLeft } = this;
      ...
      if (this.fixedHeader) {
        const { scrollTop } = tableContainerRef ?? 0;
    ...
    setScrollBarStatus() {
      ...
      if (tableContainerRef) {
        ...
        if (scrollWidth && clientWidth) {
          this.hasXScrollBar = scrollWidth - clientWidth ? true : false;
        }

        if (scrollHeight && clientHeight) {
          this.hasYScrollBar = scrollHeight - clientHeight ? true : false;
        }
      }
    },
    ...
    saveCellWhenStopEditing() {
      const { colgroups, rowKeyFieldName, editOption, editingCell, isCellEditing } = this;
    ...
    bodyCellMousedown({ event, rowData, column }) {
      ...
      const { editOption, rowKeyFieldName, colgroups, cellSelectionData, cellSelectionRangeData, allRowKeys } = this;
      ...
      if (isOperationColumn(colKey, colgroups)) {
        ...
        const { startRowKey, endRowKey, startRowKeyIndex, endRowKeyIndex } = bodyIndicatorRowKeys;
    ...
    headerCellMousedown({ event, column }) {
      ...
      const { isGroupHeader, colgroups, headerIndicatorColKeys, cellSelectionData } = this;
      ...
      if (shiftKey) {
        ...
        const mouseEventClickType = getMouseEventClickType(event);
        const currentCellStartColIndex = colgroups.findIndex(x => x.key === currentCellEndColKey);
        const currentCellEndColIndex = colgroups.findIndex(x => x.key === currentCellStartColKey);
        // 左键点击 || 不在当前选择列内
        if (
          mouseEventClickType === MOUSE_EVENT_CLICK_TYPE.LEFT_MOUSE ||
    ...
    headerCellMouseover({ event, column }) {
      const { colgroups, isGroupHeader, isHeaderCellMousedown, headerIndicatorColKeys } = this;

      if (isHeaderCellMousedown && !isOperationColumn(column.key, colgroups)) {
        let colKeys;
        if (isGroupHeader) {
          colKeys = getColKeysByHeaderColumn({
            headerColumnItem: column,
    ...
    editCellByClick({ isDblclick, rowKey, colKey }) {
      const { editOption, isCellEditing, hasEditColumn, editingCell, isEditColumn } = this;
      ...
      if (editingCell && editingCell.rowKey == rowKey && editingCell.colKey == colKey) {
        return false;
      }
      if (isCellEditing) {
        this[INSTANCE_METHODS.STOP_EDITING_CELL]();
    ...
    bodyContextmenuItemClick(type) {
      ...
      if (!isEmptyValue(rowKey) && !isEmptyValue(colKey)) {
        ...
        // insert row above
          ...
          tableData.splice(currentRowIndex, 0, createEmptyRowData({ colgroups, rowKeyFieldName }));
        }
        // insert row below
        else if (CONTEXTMENU_NODE_TYPES.INSERT_ROW_BELOW === type) {
          tableData.splice(currentRowIndex + 1, 0, createEmptyRowData({ colgroups, rowKeyFieldName }));
        }
      }
    },
    ...
    /*
        set cell selection and column to visible
        */
    [INSTANCE_METHODS.SET_CELL_SELECTION]({ rowKey, colKey, isScrollToRow = true }) {
    ...
    [INSTANCE_METHODS.GET_RANGE_CELL_SELECTION]() {
      const { cellSelectionData, cellSelectionRangeData, allRowKeys, colgroups } = this;
    ...
    [INSTANCE_METHODS.SET_ALL_CELL_SELECTION]() {
      ...
      if (colgroups.length) {
        const colKeys = colgroups.filter(x => !x.operationColumn).map(x => x.key);
    ...
    [INSTANCE_METHODS.HIDE_COLUMNS_BY_KEYS](keys) {
      if (!isEmptyArray(keys)) {
        /*
                ...
                */
        this.hiddenColumns = Array.from(new Set(this.hiddenColumns.concat(keys)));
    ...
    [INSTANCE_METHODS.SCROLL_TO_ROW_KEY]({ rowKey }) {
      ...
      if (isVirtualScroll) {
        const position = this.virtualScrollPositions.find(x => x.rowKey === rowKey);
        ...
        const rowEl = this.$el.querySelector(`tbody tr[${COMPS_CUSTOM_ATTRS.BODY_ROW_KEY}="${rowKey}"]`);
    ...
    // start editing cell
    [INSTANCE_METHODS.START_EDITING_CELL]({ rowKey, colKey, defaultValue }) {
      const { editOption, colgroups, rowKeyFieldName, editingCell, cellSelectionData } = this;
      ...
      if (cellSelectionData.currentCell.colKey !== colKey || cellSelectionData.currentCell.rowKey !== rowKey) {
        this.cellSelectionCurrentCellChange({
          rowKey,
          colKey,
        });
  ...
  created() {
    // bug fixed #467
    this.debouncedBodyCellWidthChange = debounce(this.bodyCellWidthChange, 0);
  },
  mounted() {
    this.parentRendered = true;

    // set contextmenu event target
    this.contextmenuEventTarget = this.$el.querySelector(`.${clsName('content')}`);
    ...
    this.$on(EMIT_EVENTS.HEADER_ROW_HEIGHT_CHANGE, ({ rowIndex, height }) => {
    ...
    this.$on(EMIT_EVENTS.FOOTER_ROW_HEIGHT_CHANGE, ({ rowIndex, height }) => {
  ...
  render() {
    ...
    const headerProps = {
      ...
      style: {
        cursor: this.isColumnResizerHover || this.isColumnResizing ? 'col-resize' : '',
      },
    ...
    const bodyProps = {
      ...
      bodyIndicatorRowKeys,
      ...convertToV3Event({
        [EMIT_EVENTS.BODY_CELL_WIDTH_CHANGE]: debouncedBodyCellWidthChange,
        [EMIT_EVENTS.HIGHLIGHT_ROW_CHANGE]: this[INSTANCE_METHODS.SET_HIGHLIGHT_ROW],
      }),
    };
    ...
    const tableContainerProps = {
      ...
      ...convertToV3Event({
        scroll: () => {
          ...
          this.hooks.triggerHook(HOOKS_NAME.TABLE_CONTAINER_SCROLL, tableContainerRef);
          this.setScrolling(tableContainerRef);

          if (isVirtualScroll) {
            this.tableContainerVirtualScrollHandler(tableContainerRef);

            const { virtualScrollStartIndex: startIndex, previewVirtualScrollStartIndex: previewStartIndex } = this;
    ...
    const tableWrapperProps = {
      ...
      tagName: 'div',
      'onDom-resize-change': ({ height }) => {
        console.log('onDom-resize-change', height)
          ...
          <VueDomResizeObserver {...tableContainerWrapperProps}>
            <div {...tableContainerProps}>
              ...
              <VueDomResizeObserver {...tableWrapperProps}>
                ...
                {/* cell selection */}
                {enableCellSelection && <Selection {...selectionProps} />}
              </VueDomResizeObserver>
            </div>
            {/* edit input */}
            {enableCellSelection && <EditInput {...editInputProps} />}
            {/* contextmenu */}
            {(this.enableHeaderContextmenu || this.enableBodyContextmenu) && <VeContextmenu {...contextmenuProps} />}
            {/* column resizer */}
            {enableColumnResize && <ColumnResizer {...columnResizerProps} />}
          </VueDomResizeObserver>,
          [[clickoutside, this.tableClickOutside]],
        )}
      </div>
    );
  },
