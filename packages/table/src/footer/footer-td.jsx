...
export default {
  ...
  methods: {
    ...
    getBodyTdClass({ fixed }) {
      ...
      const { cellStyleOption, rowData, column, rowIndex, cellSelectionData, rowKeyFieldName } = this;

      // column fixed
      if (fixed) {
        result[clsName('fixed-left')] = fixed === 'left';
        result[clsName('fixed-right')] = fixed === 'right';
        result[clsName('last-left-fixed-column')] = this.isLastLeftFixedColumn;
        result[clsName('first-right-fixed-column')] = this.isfirstRightFixedColumn;
      }
  ...
  render(h) {
    ...
    if (eventCustomOption) {
      ...
      customEvents = footerCellEvents ? footerCellEvents({ row: rowData, column, rowIndex }) : {};
    }
