...
import { getLastZIndex, nextZIndex, isEnableConf } from '../ui/utils';
import { getOffsetHeight, getPaddingTopBottomSize, getDomNode } from '../ui/dom';
import { VxeUI } from '../ui';
import VxeTableComponent from '../main';
import VxeToolbarComponent from '../toolbar';
import tableComponentProps from '../main/props';
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const refTable = ref() as Ref<
      ComponentPublicInstance<VxeTableProps, VxeTableConstructor & VxeTableMethods & VxeTablePrivateMethods>
    >;
    ...
    const computeProxyOpts = computed(() => {
      return XEUtils.merge(
        {},
        XEUtils.clone(getConfig().grid.proxyConfig, true),
        props.proxyConfig,
    ...
    const computeFormOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().grid.formConfig, true),
        props.formConfig,
      ) as VxeGridPropTypes.FormOpts;
    });

    const computeToolbarOpts = computed(() => {
      return Object.assign(
        {},
        XEUtils.clone(getConfig().grid.toolbarConfig, true),
        props.toolbarConfig,
      ) as VxeGridPropTypes.ToolbarOpts;
    });
    ...
    const getRespMsg = (rest: any, defaultMsg: string) => {
      ...
      let msg;
      if (rest && messageProp) {
        msg = XEUtils.isFunction(messageProp)
          ? messageProp({ data: rest, $grid: $xeGrid })
          : XEUtils.get(rest, messageProp);
      }
    ...
    const dispatchEvent = (type, params: Record<string, any>, evnt: Event | null) => {
      ...
      getExcludeHeight() {
        ...
        const parentPaddingSize =
          isZMax || !(height === 'auto' || height === '100%')
            ? 0
            : getPaddingTopBottomSize(el.parentNode as HTMLElement);
        return (
          parentPaddingSize +
        ...
        if (el) {
          return (
            (reactData.isZMax
              ...
              : XEUtils.toNumber(getComputedStyle(el.parentNode as HTMLElement).height)) -
