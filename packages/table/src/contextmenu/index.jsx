...
import VeIcon from '../table-icon';
import { ICON_NAMES } from '../utils/constant';
import { getMousePosition, getViewportOffset } from '../utils/dom';
import { INIT_DATA, EMIT_EVENTS, CONTEXTMENU_NODE_TYPES, INSTANCE_METHODS } from './util/constant';
import { getRandomId } from '../utils/random';
import { debounce, cloneDeep } from 'lodash';
import eventsOutside from '../directives/events-outside';
...
export default {
  ...
  methods: {
    ...
    showContextmenuPanel({ event, contextmenuId, isRootContextmenu }) {
      ...
      if (contextmenuPanelEl) {
        ...
        const { width: currentPanelWidth, height: currentPanelHeight } = contextmenuPanelEl.getBoundingClientRect();
        if (isRootContextmenu) {
          const { left: clickLeft, top: clickTop, right: clickRight, bottom: clickBottom } = getMousePosition(event);
          ...
          if (parentContextmenuPanelEl) {
            const { left: parentPanelLeft, right: parentPanelRight } = getViewportOffset(parentContextmenuPanelEl);
  ...
  render() {
    const { panelOptions, activeMenuIds, hasChildren, emptyContextmenuPanels, debounceCreatePanelByHover } = this;
                ...
                {panelOption.menus.map(menu => {
                  ...
                  if (menu.type !== CONTEXTMENU_NODE_TYPES.SEPARATOR) {
                    contextmenuNodeProps = {
                      class: {
                        [clsName('node')]: true,
                        [clsName('node-active')]: activeMenuIds.includes(menu.id),
                        [clsName('node-disabled')]: menu.disabled,
                      },
                      ...convertToV3Event({
                        mouseover: event => {
                          // disable
                        ...
                        click: () => {
                          if (!menu.disabled && !hasChildren(menu)) {
                            this.$emit(EMIT_EVENTS.ON_NODE_CLICK, menu.type);
                            setTimeout(() => {
                              emptyContextmenuPanels();
                            }, 50);
                          }
                  ...
                  if (menu.type !== CONTEXTMENU_NODE_TYPES.SEPARATOR) {
                    return (
                      <li {...contextmenuNodeProps}>
                        <span class={clsName('node-label')}>{menu.label}</span>
                        {menu.hasChildren && (
                          <VeIcon
