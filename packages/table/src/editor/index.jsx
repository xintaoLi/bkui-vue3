...
import { autoResize } from '../utils/auto-resize';
import { isEmptyValue } from '../utils/index.js';
import { getCaretPosition, setCaretPosition } from '../utils/dom';
...
export default {
  ...
  computed: {
    ...
    containerStyle() {
      ...
      const { displayTextarea, overflowViewport, textareaRect, currentColumn: column } = this;
  ...
  methods: {
    ...
    setCellEl() {
      ...
      if (tableEl) {
        const cellEl = tableEl.querySelector(`tbody.ve-table-body tr[row-key="${rowKey}"] td[col-key="${colKey}"]`);
  ...
  render() {
    const { containerClass, containerStyle, textareaClass, rawCellValue, isCellEditing, isEditCellFocus } = this;
    ...
    return (
      <div {...containerProps}>
        {withDirectives(<textarea {...textareaProps}></textarea>, [
          [
            focus,
            {
