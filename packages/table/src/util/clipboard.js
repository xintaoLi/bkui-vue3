 ...
 */
export function onBeforeCopy({ cellSelectionRangeData, selectionRangeData, colgroups, allRowKeys }) {
 ...
 * @return
 */
export function onBeforePaste({ event, cellSelectionRangeData, colgroups, allRowKeys }) {
  let pastedData;
  ...
  const startColIndex = colgroups.findIndex(x => x.key === cellSelectionRangeData.leftColKey);
  const endColIndex = Math.min(startColIndex + decodePasteData[0].length - 1, colgroups.length - 1);
  ...
  const sourceFieldNames = colgroups.slice(startColIndex, endColIndex + 1).map(x => x.field);

  response.data = decodePasteData.slice(0, endRowIndex - startRowIndex + 1).map(rowData => {
    let newRow = {};
...
export function onAfterPaste({ tableData, beforePasteResponse }) {
  ...
  pasteData.forEach((rowData, rowIndex) => {
    Object.assign(tableData[selectionRangeIndexes.startRowIndex + rowIndex], rowData);
  });
}
 ...
 * @return {selectionRangeIndexes,selectionRangeKeys,data}
 */
export function onBeforeCut({ cellSelectionRangeData, selectionRangeData, colgroups, allRowKeys }) {
  const { leftColKey, rightColKey, topRowKey, bottomRowKey } = cellSelectionRangeData;
 ...
 * @return
 */
export function onAfterCut({ event, tableData, colgroups, selectionRangeData, selectionRangeIndexes }) {
  const spreadsheetStr = encodeToSpreadsheetStr(selectionRangeData);
 ...
 * @return {selectionRangeIndexes,selectionRangeKeys,data}
 */
export function onBeforeDelete({ cellSelectionRangeData, selectionRangeData, colgroups, allRowKeys }) {
  const { leftColKey, rightColKey, topRowKey, bottomRowKey } = cellSelectionRangeData;
