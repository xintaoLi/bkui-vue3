import { PREFIX_CLS, CONTEXTMENU_NODE_TYPES, COLUMN_FIXED_TYPE, AUTOFILLING_DIRECTION } from './constant';
import { MOUSE_EVENT_CLICK_TYPE } from '../utils/constant';
import { isEmptyValue, isEmptyArray, isFunction } from '../utils/index';
...
export function getFixedTotalWidthByColumnKey({ colgroups, colKey, fixed }) {
  ...
  if (fixed === COLUMN_FIXED_TYPE.LEFT) {
    ...
    result = colgroups.reduce((total, currentVal, index) => {
      return index < currentIndex && currentVal.fixed == COLUMN_FIXED_TYPE.LEFT
    ...
    // 只计算右列固定的
    result = colgroups.reduce((total, currentVal, index) => {
      return index > currentIndex && currentVal.fixed == COLUMN_FIXED_TYPE.RIGHT
        ? currentVal._realTimeWidth + total
        : total;
    }, 0);
  }
...
export function setHeaderContextmenuOptions({
  ...
  if (enableHeaderContextmenu) {
    ...
    const colCount = selectionRangeIndexes.endColIndex - selectionRangeIndexes.startColIndex + 1;

    const { contextmenus, beforeShow } = contextmenuHeaderOption;

    const isWholeColSelection = !isEmptyValue(headerIndicatorColKeys.startColKey);
    ...
    contextmenus.forEach(contextmenu => {
      const contentmenuCollectionItem = headerContextmenuOptionCollection.find(x => x.type === contextmenu.type);
      if (contentmenuCollectionItem) {
        let isContinue = true;
        // empty column. 选中整列时支持
        if (contentmenuCollectionItem.type === CONTEXTMENU_NODE_TYPES.EMPTY_COLUMN) {
        ...
        else if (contentmenuCollectionItem.type === CONTEXTMENU_NODE_TYPES.LEFT_FIXED_COLUMN_TO) {
          //
          if (isOperationCol) {
        ...
        else if (contentmenuCollectionItem.type === CONTEXTMENU_NODE_TYPES.CANCEL_LEFT_FIXED_COLUMN_TO) {
          if (leftFixedColKeys.length < 1) {
            contentmenuCollectionItem.disabled = true;
          }
        }
        // right fixed column to
        else if (contentmenuCollectionItem.type === CONTEXTMENU_NODE_TYPES.RIGHT_FIXED_COLUMN_TO) {
          //
          if (isOperationCol) {
            contentmenuCollectionItem.disabled = true;
          }
        ...
        else if (contentmenuCollectionItem.type === CONTEXTMENU_NODE_TYPES.CANCEL_RIGHT_FIXED_COLUMN_TO) {
...
export function setBodyContextmenuOptions({
  ...
  if (enableBodyContextmenu) {
    ...
    const rowCount = selectionRangeIndexes.endRowIndex - selectionRangeIndexes.startRowIndex + 1;
    const colCount = selectionRangeIndexes.endColIndex - selectionRangeIndexes.startColIndex + 1;
    ...
    contextmenus.forEach(contextmenu => {
      const contentmenuCollectionItem = bodyContextmenuOptionCollection.find(x => x.type === contextmenu.type);
      if (contentmenuCollectionItem) {
        let isContinue = true;

        // remove row
        if (contentmenuCollectionItem.type === CONTEXTMENU_NODE_TYPES.REMOVE_ROW) {
          contentmenuCollectionItem.label = contentmenuCollectionItem.label.replace('$1', rowCount);
        }
        // empty row. 选中整行时支持
        else if (contentmenuCollectionItem.type === CONTEXTMENU_NODE_TYPES.EMPTY_ROW) {
        ...
        // empty cell.没选中整行时支持
        else if (contentmenuCollectionItem.type === CONTEXTMENU_NODE_TYPES.EMPTY_CELL) {
          isContinue = !isWholeRowSelection;
        }
        // remove column.没选中整行时支持
        else if (contentmenuCollectionItem.type === CONTEXTMENU_NODE_TYPES.REMOVE_COLUMN) {
          if (isWholeRowSelection) {
            isContinue = false;
          } else {
            contentmenuCollectionItem.label = contentmenuCollectionItem.label.replace('$1', colCount);
...
export function getColKeysByFixedTypeWithinColKeys({ colKeys, fixedType, colgroups }) {
  ...
  if (Array.isArray(colKeys)) {
    result = colgroups.filter(x => colKeys.indexOf(x.key) > -1 && x.fixed === fixedType).map(x => x.key);
  }
 ...
 * @return {Array<colKeys>}
 */
export function isCellInSelectionRange({ cellData, cellSelectionRangeData, colgroups, allRowKeys }) {
...
export function getSelectionRangeData({
  ...
  const fieldNames = colgroups.slice(startColIndex, endColIndex + 1).map(x => x.field);

  if (resultType === 'normal') {
    result = tableData.slice(startRowIndex, endRowIndex + 1).map(rowData => {
      let newRow = {};
...
function getLeftmostOrRightmostColKey({ type, colgroups, colKeys }) {
  ...
  if (Array.isArray(colKeys) && colKeys.length) {
    ...
    colKeys.forEach(colKey => {
      ...
      if (colIndex === -1) {
        console.warn(`getLeftmostOrRightmostColKey error:: can't find colKey:${colKey}`);
        return false;
      }
...
export function cellAutofill({
  ...
  sourceSelectionRangeIndexes.startColIndex = colgroups.findIndex(x => x.key === leftColKey);
  sourceSelectionRangeIndexes.endColIndex = colgroups.findIndex(x => x.key === rightColKey);
  ...
  if (direction === AUTOFILLING_DIRECTION.UP) {
    ...
    if (isReplaceData) {
      ...
      for (
        ...
        for (
          ...
          // repeat autofill cell selection data
          if (cellSelectionTableDataRowIndex > cellSelectionTableData.length - 1) {
    ...
    targetSelectionRangeIndexes.startColIndex = colgroups.findIndex(x => x.key === nextCurrentCell.colKey);
    ...
    if (isReplaceData) {
      ...
      for (
        ...
        let cellSelectionTableDataColIndex = sourceSelectionRangeIndexes.endColIndex;
        ...
        for (
          ...
          tableData[rowIndex][fieldName] =
            cellSelectionTableData[cellSelectionTableDataRowIndex][colgroups[cellSelectionTableDataColIndex].field];
    ...
    targetSelectionRangeIndexes.endColIndex = colgroups.findIndex(x => x.key === nextNormalEndCell.colKey);
    ...
    if (isReplaceData) {
      ...
      for (
        ...
        let cellSelectionTableDataColIndex = sourceSelectionRangeIndexes.startColIndex;
        ...
        for (
          ...
          if (
            ...
            sourceSelectionRangeIndexes.startColIndex +
              (sourceSelectionRangeIndexes.endColIndex - sourceSelectionRangeIndexes.startColIndex)
          ) {
            cellSelectionTableDataColIndex = sourceSelectionRangeIndexes.startColIndex;
          }
  ...
  const sourceFieldNames = colgroups
    .slice(sourceSelectionRangeIndexes.startColIndex, sourceSelectionRangeIndexes.endColIndex + 1)
    .map(x => x.field);
  response.sourceSelectionData = tableData
    .slice(sourceSelectionRangeIndexes.startRowIndex, sourceSelectionRangeIndexes.endRowIndex + 1)
    .map(rowData => {
      let newData = {
  ...
  const targetFieldNames = colgroups
    .slice(targetSelectionRangeIndexes.startColIndex, targetSelectionRangeIndexes.endColIndex + 1)
    .map(x => x.field);
  response.targetSelectionData = tableData
    .slice(targetSelectionRangeIndexes.startRowIndex, targetSelectionRangeIndexes.endRowIndex + 1)
    .map(rowData => {
      let newData = {
        [rowKeyFieldName]: rowData[rowKeyFieldName],
      };
 ...
 * @return cloneColumns
 */
export function setColumnFixed({ cloneColumns, cellSelectionRangeData, fixedType, colgroups, enableColumnResize }) {
  ...
  if (fixedColIndex > -1) {
    ...
    result = cloneColumns.map((colItem, index) => {
      ...
      if (enableColumnResize && !(Array.isArray(colItem.children) && colItem.children.length)) {
        const _colItem = colgroups.find(x => x.key === colItem.key && !isEmptyValue(x.key));
        if (_colItem) {
