...
import { getViewportOffset, getViewportOffsetWithinContainer } from '../utils/dom';
...
export default {
  ...
  methods: {
    ...
    beforeVisibleChangeCallback(nextVisible) {
      ...
      if (nextVisible !== isDropdownVisible && isFunction(beforeVisibleChange)) {
        // next visible
    ...
    changDropdownPanelPosition() {
      const { defaultInstance, rootId, popperAppendToEl, appendToElTagName } = this;

      let rootEl = document.querySelector(`#${rootId}`);

      if (rootEl) {
        const { width: currentPanelWidth, height: currentPanelHeight } = rootEl.getBoundingClientRect();
        ...
        if (triggerElBottom >= currentPanelHeight) {
          panelY = triggerElTop + triggerElHeight + defaultInstance + scrollTop;
        }
        // 下方高度不够显示在鼠标点击上方
        else {
          panelY = triggerElTop - currentPanelHeight - defaultInstance + scrollTop;
        }
    ...
    addRootElementToElement() {
      ...
      if (rootEl) {
        ...
        this.$nextTick(() => {
          ...
          if (typeof popperAppendTo === 'string' && popperAppendTo.length > 0) {
            this.popperAppendToEl = document.querySelector(popperAppendTo);
          } else {
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      const targetEl = this.appendToElTagName === 'BODY' ? document : this.popperAppendToEl;
      targetEl.addEventListener('scroll', this.changDropdownPanelPosition);
    });
  ...
  destroyed() {
    ...
    this.$nextTick(() => {
      const targetEl = this.appendToElTagName === 'BODY' ? document : this.popperAppendToEl;
      targetEl.removeEventListener('scroll', this.changDropdownPanelPosition);
    });
  ...
  render() {
    ...
    if (isMultiple) {
      content = internalOptions.map((item, index) => {
        const checkboxProps = {
          ...
          ...convertToV3Event({
            change: isChecked => this.checkedChangeControl(item, isChecked),
          }),
        };
        ...
        return (
          <li
            key={index}
            class={[clsName('items-multiple'), clsName('items-li'), getTextAlignClass()]}
          >
      ...
      content = internalOptions.map((item, index) => {
        ...
        return (
          <li
            key={index}
            class={[clsName('items-li'), item.selected ? 'active' : '']}
          ...
          >
            ...
            >
              {this.showRadio ? <Radio {...radioProps}>{item.label}</Radio> : item.label}
            </a>
          </li>
        );
      });
    ...
    return (
      <dl {...dropdownProps}>
        <dt
          class='ve-dropdown-dt'
          onClick={dropdownPanelClick}
        >
          <a
                  ...
                  <div>
                    ...
                    {showOperation && (
                      <li class={clsName('operation')}>
                        <a
                          class={[clsName('operation-item'), this.operationFilterClass]}
                          href='javascript:void(0)'
                          onClick={reset}
                        >
