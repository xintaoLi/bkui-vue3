...
import { getFixedTotalWidthByColumnKey, clsName, getColKeysByRangeColKeys, getColKeysByHeaderColumn } from '../util';
import { getValByUnit, isEmptyValue } from '../utils/index.js';
...
export default {
    ...
    getSortContent() {
      ...
      if (Object.keys(sortColumns).includes(currentField)) {
        ...
        result = (
          <span {...props}>
            <TableIcon
              class={[clsName('sort-icon'), clsName('sort-icon-top'), sortBy === 'asc' ? 'active' : '']}
              name={ICON_NAMES.SORT_TOP_ARROW}
            />
            <TableIcon
              class={[clsName('sort-icon'), clsName('sort-icon-bottom'), sortBy === 'desc' ? 'active' : '']}
  ...
  render(h) {
    const { groupColumnItem, getTheadThClass, getTheadThStyle, rowIndex, eventCustomOption } = this;
    ...
    const events = {
      click: e => {
        ...
        if (this.isSortableCloumn && e.target instanceof HTMLTableCellElement) {
          this.sortChange();
        }
      },
      dblclick: e => {
